...
output_train=train_output(n(1:1900));     %一行
%input_test=test_input(n(1901:2000),:)';
%output_test=test_output(n(1901:2000));

[inputn,inputps]=mapminmax(input_train);          %输入归一化
[outputn,outputps]=mapminmax(output_train);    %输出归一化
...
for t = 2:Length
    ...
    [q_1(t,:),eInt(t,:)] = MahonyAHRSupdate( Data(t,27:29), Data(t,9:11), Data(t,15:17),q_1(t-1, :),SamplePeriod,Ki,Kp,eInt(t-1,:));%Mahony法
    tt(t)=toc;%计算算法运行时间
	...
	%%Mahony法计算姿态
	%[q1(t,:),eInt(t,:)] = MahonyAHRSupdate( Data(t,27:29), Data(t,9:11), Data(t,15:17),q(t-1, :),SamplePeriod,Ki,Kp,eInt(t-1,:));%Mahony法
   ...
   % Rn(t-1)=Re/sqrt(1-e*e*sin(L(t-1))*sin(L(t-1)));               %卯酉曲率半径
    ...
    BPoutput(t,:)=mapminmax('reverse',an,outputps);%按照outputps格式进行反归一化，变成真实值
...
xlabel('Time (s)');
ylabel('Angle (deg)');
legend('Roll梯度算法计算值','Roll参考值', 'Roll Mahony法计算值');
hold off;
...
xlabel('Time (s)');
ylabel('Angle (deg)');
legend('Pitch梯度算法计算值','Pitch参考值', 'Pitch Mahony法计算值');
hold off;
...
xlabel('Time (s)');
ylabel('Angle (deg)');
legend('Yaw梯度算法计算值','Yaw参考值', 'Yaw Mahony法计算值');
hold off;
...
d=legend('北向速度计算值', '北向速度参考值','elm北向速度值');
set(d,'Fontsize',10,'LineWidth',2,'box','off');
hold off;
figure('Name', '5');
hold on;
...
d=legend('东向速度计算值', '东向速度参考值','elm东向速度值');
set(d,'Fontsize',10,'LineWidth',2,'box','off');
hold off;
figure('Name', '6');
hold on;
...
d=legend('地向速度计算值', '地向速度参考值','elm地向速度值');
set(d,'Fontsize',10,'LineWidth',2,'box','off');
hold off;
